...
<head>
    ...
    <title>Masuk Akun Premium - Dapoer Budess</title>
        ...
        <div class="bg-white rounded-3xl premium-shadow overflow-hidden border border-orange-100">
            ...
            <div class="bg-dark-brown p-8 text-center relative overflow-hidden">
                ...
                <div class="relative z-10">
                    <h1 class="text-3xl font-playfair text-gold mb-2">Selamat Datang</h1>
                    <p class="text-cream opacity-80 font-light">Masuk untuk melanjutkan pesanan Anda</p>
                </div>
            </div>

            <div class="p-8">
                @if(session('success'))
                <div class="mb-6 p-4 bg-green-50 border border-green-100 text-green-700 rounded-xl text-sm font-semibold flex items-center animate-pulse">
                    <i class="fas fa-check-circle mr-3 text-green-500 text-lg"></i>{{ session('success') }}
                </div>
                @endif

                @if(session('error'))
                <div class="mb-6 p-4 bg-red-50 border border-red-100 text-red-700 rounded-xl text-sm font-semibold flex items-center">
                ...
                <div class="relative mb-8">
                    ...
                    <div class="relative flex justify-center text-xs uppercase tracking-widest">
                        <span class="px-4 bg-white text-gray-400 font-bold">Atau email</span>
                    </div>
                </div>

                <!-- Login Form -->
                <form method="POST" action="{{ route('customer.login.post') }}" class="space-y-6" id="login-form">
                    @csrf

                    <div>
                        <label for="email" class="block text-xs font-bold text-dark-brown uppercase tracking-wider mb-2 ml-1">Email</label>
                        <div class="relative">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2 ml-1">
                            <label for="password" class="block text-xs font-bold text-dark-brown uppercase tracking-wider">Password</label>
                            <a href="#" class="text-xs text-soft-brown hover:text-dark-brown font-bold">Lupa?</a>
                        </div>
                        <div class="relative">
                            <span class="absolute inset-y-0 left-0 pl-4 flex items-center text-gray-400">
                    ...
                    <div class="flex items-center">
                        <input type="checkbox" name="remember" id="remember" class="w-4 h-4 text-dark-brown border-gray-300 rounded focus:ring-dark-brown cursor-pointer">
                        <label for="remember" class="ml-2 text-sm text-soft-brown font-medium cursor-pointer">Ingat saya</label>
                    </div>

                    <button type="submit" id="submit-btn" class="btn-premium w-full py-3.5 rounded-xl font-bold text-lg shadow-lg mt-2 flex items-center justify-center">
                ...
                <p class="mt-8 text-center text-gray-500 text-sm">
                    Belum memiliki akun? 
                    <a href="{{ route('customer.register') }}" class="text-dark-brown font-bold hover:text-gold transition-colors">Daftar Gratis</a>
                </p>
            </div>
        </div>
    </div>
